...
import static com.github.FurianMan.time_tracker.Constants.getUserLogger;
...
public class GetUser {
     ...
     */
    public static TableUsers getUser(TableUsers userForSearch) {
...
//        Utilities.validateUserFields(userForSearch); // не ясно нужно ли валидировать пользователя при GET. если да, то надо логически вставить функцию
        ...
        if (user_id != 0) {
            ...
            try {
                ...
                getUserLogger.debug(String.format(sqlQuery));
                resSet = statmt.executeQuery(sqlQuery);
                if (!resSet.next()) {
                    getUserLogger.error(String.format("Cannot find in database user: user_id=%d", user_id));
                ...
                getUserLogger.info(String.format("User has been found successfully: user_id=%d", user_id));
            } catch (SQLException e) {
                getUserLogger.error("Cannot execute query `getUser` to database", e);
            ...
            try {
                ...
                getUserLogger.debug(sqlQuery);
                resSet = statmt.executeQuery(sqlQuery);
                if (!resSet.next()) {
                    getUserLogger.error(String.format("Cannot find in database user: name=%s, surname=%s, position=%s, birthday=%s", name, surname, position, birthday));
                    throw new ApplicationException("Cannot find the user in database", 404);
                } else {
                    do { //TODO можно сделать без цикла
                        userInstance.setUser_id(resSet.getInt("user_id"));
                ...
                getUserLogger.info(String.format("User has been found successfully: name=%s, surname=%s, position=%s, birthday=%s", name, surname, position, birthday));
            } catch (SQLException e) {
                getUserLogger.error("Cannot execute query `getUser` to database", e);
                throw new ApplicationException("Cannot execute query `getUser` to database", e, 500);
            } finally {
            ...
            try {
                ...
                resSet = statmt.executeQuery(sqlQuery);
                if (!resSet.next()) {
                    getUserLogger.error(String.format("Cannot find in database user: name=%s, surname=%s", name, surname));
                    throw new ApplicationException("Cannot find the user in database", 404);
                } else {
                    do {
                        userInstance.setUser_id(resSet.getInt("user_id"));
                ...
                getUserLogger.info(String.format("User has been found successfully: name=%s, surname=%s", name, surname));
            } catch (SQLException e) {
                getUserLogger.error("Cannot execute query `getUser` to database", e);
                throw new ApplicationException("Cannot execute query `getUser` to database", e, 500);
            } finally {
            ...
            getUserLogger.error("Request does not have required fields for method `getUser`");
            getUserLogger.debug(String.format("Request does not have required fields for method `getUser`. " +
