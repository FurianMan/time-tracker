...
import static com.github.FurianMan.time_tracker.Constants.getTaskLogger;
...
public class GetTask {
     ...
     */
    public static TableTasks getTask(TableTasks taskForSearch) {
        ...
        if (user_id != 0 && task_num != 0) { //TODO на самом деле этим методом пользуется только insertTask и там проверки эти уже есть
            ...
            try {
                ...
                getTaskLogger.debug(String.format(sqlQuery));
                if (!resSet.next()) {
                    getTaskLogger.error(String.format("Cannot find in database task: user_id=%d task_num=%d start_time=%s", user_id, task_num, start_time));
                ...
                getTaskLogger.info(String.format("Task has been found successfully: user_id=%d task_num=%d start_time=%s", user_id, task_num, start_time));
            } catch (SQLException e) {
                getTaskLogger.error("Cannot execute query `getTask` to database", e);
            ...
            getTaskLogger.error("Request does not have required fields for method `getTask`");
            getTaskLogger.debug(String.format("Received fields in `getTask`:  user_id=%d, task_num=%d, start_time=%s", user_id, task_num, start_time));
